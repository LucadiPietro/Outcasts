...
namespace RhythmCombat.Domain.Geometry
{
    public sealed class BattlefieldLayout
    {
        ...
        private readonly Dictionary<int, Double2> _spawnCentersByLane;

        public BattlefieldLayout(Double2 spawnCenter, IEnumerable<LaneGeometry> lanes)
            : this(spawnCenter, lanes, null)
        {
        }

        public BattlefieldLayout(Double2 spawnCenter, IEnumerable<LaneGeometry> lanes, IDictionary<int, Double2> spawnCentersByLane)
        {
            if (lanes is null)
            ...
            _spawnCentersByLane = spawnCentersByLane != null
                ? new Dictionary<int, Double2>(spawnCentersByLane)
                : new Dictionary<int, Double2>();
        ...
        public Double2 GetSpawnCenter(int laneIndex)
        {
            LaneMapping.ValidateLaneIndex(laneIndex);

            if (_spawnCentersByLane.TryGetValue(laneIndex, out var laneSpawnCenter))
                return laneSpawnCenter;
        ...
        public static BattlefieldLayout CreateLaneCenteredStandard(double laneSpacing = 2d, double rowOffset = 1d, double toleranceRadius = 1d)
        {
            var lanes = new[]
            {
                new LaneGeometry(0, new Double2(-laneSpacing, rowOffset), toleranceRadius),
            ...
            var spawnCenters = new Dictionary<int, Double2>
            {
                { 0, new Double2(-laneSpacing, 0d) },
                { 1, new Double2(0d, 0d) },
                { 2, new Double2(laneSpacing, 0d) },
                { 3, new Double2(-laneSpacing, 0d) },
                { 4, new Double2(0d, 0d) },
                { 5, new Double2(laneSpacing, 0d) }
            };

            return new BattlefieldLayout(new Double2(0d, 0d), lanes, spawnCenters);
        }
    }
}
